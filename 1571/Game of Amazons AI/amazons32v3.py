...
class ate9_Board:
	...
	def moves(self, is_white):
		...
		for queen_move in queen_moves:
			...
			moved_board = self.move_the_queen(self.board, source, destination)
	...
	@staticmethod
	def move_the_queen(board, src, dst):
		copy = np.copy(board)
		copy[dst] = copy[src]
		copy[src] = '.'
		return ate9_Board(copy)
...
def generate_moves(ate9_board, board, player):
	moves = ate9_board.moves(player)
	boards = []
	
	for move in moves:
		temp_board = copy.deepcopy(board)
		source, destin, arrow = move
		temp_board.move_queen(source, destin)
		temp_board.shoot_arrow(arrow)
		boards.append(temp_board)
		
	return moves, boards
	
def ate9_heuristic(ate9_board, board, player):
	score = generate_moves(ate9_board, board, player).__len__()
	return score
	
def ate9_getMax(board, is_white, alpha, beta, depth, time_limit):
	value = -inf
	best_move = None
	start_time = time.time()
	limit = time_limit
	
	move_generation = generate_moves(ate9_Board(np.array(board.config)), board, is_white)[0]
	for move in move_generation:
		end_time = time.time()
		...
		if (depth == 1):
			score = ate9_heuristic(ate9_Board(np.array(temp_board.config)), board, is_white)
			if (score > value):
				value = score
				best_move = move
				return value, best_move
		min_result = ate9_getMin(temp_board, not is_white, alpha, beta, depth-1, limit)[0]
		if (min_result > value):
			value = min_result
			best_move = move
...
def ate9_getMin(board, is_white, alpha, beta, depth, time_limit):
	value = inf
	best_move = None
	start_time = time.time()
	limit = time_limit
	...
	for move in move_generation:
		...
		if(depth == 1):
			...
			if score < value:
				...
				best_move = move
				return value, best_move
		max_result = ate9_getMax(temp_board, not is_white, alpha, beta, depth-1, limit)[0]
		if max_result > value:
			value = max_result
			best_move = move
			beta = min(beta, value)
...
def ate9_MinimaxAB(board, is_white, alpha, beta, depth, time_limit):
	return ate9_getMax(board, is_white, alpha, beta, depth, time_limit)
	
def ate9(board):
		is_white = board.bWhite
		ate9_board = ate9_Board(np.array(board.config))
		depth = 8
		config_file = open('amazons.config', "r")
		time_limit = ast.literal_eval(config_file.readline())
		
		print(board.print_board())
		value, best_move = ate9_getMax(board, is_white, -inf, inf, depth, time_limit)
		return best_move
