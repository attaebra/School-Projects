             ...
             implements SearchTreeInterface<T>
{
   ...
   private BinaryNode<T> removeLargest(BinaryNode<T> rootNode)
   {
	...
	BinaryNode<T>answer = getMax1(rootNode);
	
	return (T)answer.getData();
	}
	
	private BinaryNode<T> getMax1(BinaryNode<T> mainNode) //go all the way to the right to get max
	{
	if(mainNode.hasRightChild())
	{
	mainNode = mainNode.getRightChild();
	mainNode = getMax1(mainNode);
	}
	...
	{
	BinaryNode<T>rootNode = getRootNode();
	BinaryNode<T>answer = getMin1(rootNode);
	
	return (T)answer.getData();
	}
	
	private BinaryNode<T> getMin1(BinaryNode<T> mainNode)//go all the way left until at the lowest node
	{
	if(mainNode.hasLeftChild())
	{
	mainNode = mainNode.getLeftChild();
	mainNode = getMin1(mainNode);
	}
